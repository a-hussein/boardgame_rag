...
from rank_bm25 import (
    BM25Okapi,  # lexical ranking (read more here: https://pypi.org/project/rank-bm25/)
)
from sentence_transformers import (
    SentenceTransformer,  # embeddings (read more here: https://sbert.net/)
)
...
def build_faiss(embs: np.ndarray):
    ...
    index = faiss.IndexFlatIP(
        embs.shape[1]
    )  # inner product between unit vecors is cosine similarity
...
def main():
    ...
    embs = model.encode(
        fields, batch_size=64, show_progress_bar=True, normalize_embeddings=False
    )
