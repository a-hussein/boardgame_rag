...
def _rand_text(
    name: str, mechs: List[str], cats: List[str], weight: float, t: int
) -> str:
    bits = []
    ...
    return (
        f"{name} blends {', '.join(mechs[:2])} within a {cats[0].lower()} frame. "
...
def load_bgg_csv(path: pathlib.Path) -> List[Dict[str, Any]]:
    ...
    with path.open(newline="", encoding="utf-8") as f:
        ...
        for i, rec in enumerate(r):
            ...
            mechs = [
                m.strip()
                for m in (rec.get("mechanics", "").replace(";", ",").split(","))
                if m.strip()
            ]
            cats = [
                c.strip()
                for c in (rec.get("categories", "").replace(";", ",").split(","))
                if c.strip()
            ]
            ...
            text = _rand_text(
                name, mechs or ["Set Collection"], cats or ["Family"], weight, play_time
            )
            rows.append(
                dict(
                    doc_id=f"BGG{i:03d}",
                    ...
                    play_time=play_time,
                    weight=weight,
                    mechanics=mechs or ["Set Collection"],
                    categories=cats or ["Family"],
                    text=text,
                )
            )
    return rows
...
def mk_processed(in_jsonl: pathlib.Path, out_parquet: pathlib.Path) -> None:
    ...
    )  # enforce unique names by dropping duplicates
...
def main():
    ...
    ap.add_argument(
        "--bgg-csv", type=pathlib.Path, default=None, help="Optional BGG CSV to augment"
    )
